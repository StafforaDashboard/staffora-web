...
var MODS=[{k:"dienstnummern",l:"Dienstnummern"},{k:"teamverwaltung",l:"Teamverwaltung"},{k:"bewerbungen",l:"Bewerbungen"},{k:"robloxStaff",l:"Roblox Staff"},{k:"statusPanel",l:"Status Panel"},{k:"logs",l:"Logs"},{k:"adminCalls",l:"Admin Calls"},{k:"tickets",l:"Tickets"}];
...
async function renderPanel(){
  ...
  try{
    if(tab==="overview"){
      ...
      p.innerHTML="<h1>Übersicht</h1><p class=\"lead\">"+esc(s.systemName||"Staffora")+"</p><div class=\"grid\">"+MODS.map(function(x){return '<div class="card"><strong>'+x.l+'</strong><div style="margin-top:8px"><span class="badge '+(m[x.k]?"on":"off")+'">'+(m[x.k]?"AN":"AUS")+"</span></div></div>"}).join("")+"</div>";
    }else if(tab==="modules"){
      var m=cfg.modules||{};
      p.innerHTML="<h1>Module</h1><div class=\"card\">"+MODS.map(function(x){return '<div class="row" style="justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border)"><span>'+x.l+'</span><label><input type="checkbox" data-m="'+x.k+'" '+(m[x.k]?"checked":"")+"/> aktiv</label></div>"}).join("")+'<button class="btn btn-p" id="save-m" style="margin-top:12px">Speichern</button></div>';
      ...
      p.innerHTML="<h1>Einstellungen</h1><div class=\"card\">"+f("systemName","Systemname",s.systemName)+f("numberPrefix","Prefix",s.numberPrefix)+f("numberDigits","Stellen",s.numberDigits)+f("statusChannelId","Status-Kanal ID",s.statusChannelId)+f("statusIntervalSec","Update Intervall (s)",s.statusIntervalSec)+f("robloxGroupId","Roblox Gruppen-ID",s.robloxGroupId)+f("robloxMinRank","Min. Rang",s.robloxMinRank)+f("appChannelId","Bewerbungs-Kanal",s.appChannelId)+f("logChannelId","Log-Kanal",s.logChannelId)+'<button class="btn btn-p" id="save-s">Speichern</button></div>';
